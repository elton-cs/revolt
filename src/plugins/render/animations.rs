...
use crate::utils::constants::PLAYER_STATS_Z_HEIGHT;
...
fn setup(
    ...
    let texture = asset_server.load("cooked_by_hpmnk/Grotle_Walk-Anim_ripped_8x4f_32x.png");

    // the sprite sheet has 7 sprites arranged in a row, and they are all 24px x 24px
    let layout = TextureAtlasLayout::from_grid(UVec2::splat(32), 4, 8, None, None);
    let texture_atlas_layout = texture_atlas_layouts.add(layout);

    // the first (left-hand) sprite runs at 10 FPS
    let animation_config_1 = AnimationConfig::new(0, 3, 10);
